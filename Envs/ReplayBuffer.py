...
class ReplayBuffer:
    def __init__(self, max_len, state_dim, action_dim, if_use_per, gpu_id, state_type=torch.float32):
        ...
        buf_state_shape = (max_len,1, state_dim) if isinstance(state_dim, int) else (max_len,1, *state_dim)#################################
    ...
    def save_or_load_history(self, cwd, if_save, buffer_id=0):  # [ElegantRL.2021.11.11]###used
        save_path = f"{cwd}/buffer_{buffer_id}.npz"
        if_load = None

        if if_save:
            self.update_now_len()
            state_dim = (self.buf_state.shape[1],self.buf_state.shape[2],self.buf_state.shape[3])
            ...
            buf_state = np.empty((self.now_len, state_dim[0], state_dim[1],state_dim[2]), dtype=buf_state_data_type)
