...
class Actor(nn.Module):
    ...
    :param cd2_dim[int]: the dimension of days (the number of history)
    """
    def __init__(self, mid_dim, state_dim, action_dim, cd2_dim):
        super().__init__()
        self.cd2 = nn.Sequential(nn.Conv2d(1, 8, kernel_size=(3,1), stride=(2,1), padding=0),nn.ReLU(),
                                 nn.Conv2d(8, 16, kernel_size=(3,1), stride=(3,1), padding=0),nn.ReLU(),
                                 nn.Conv2d(16, 1, kernel_size=(3,1), stride=(3,1), padding=0),nn.ReLU()
                                 )
        self.net = nn.Sequential(nn.Linear(state_dim[1], mid_dim), nn.ReLU(),
    ...
    def forward(self, state):
        ...
        """
        output = self.cd2(state)
        output = output.reshape((state.shape[0],state.shape[3]))

        return self.net(output).tanh()  # action.tanh()
...
class Critic(nn.Module):
    ...
    def __init__(self, mid_dim, state_dim, action_dim, cd2_dim):
        ...
        self.net = nn.Sequential(nn.Linear(state_dim[1] + action_dim, mid_dim), nn.ReLU(),
                                 nn.Linear(mid_dim, mid_dim), nn.ReLU(),
                                 nn.Linear(mid_dim, mid_dim), nn.Hardswish(),
                                 nn.Linear(mid_dim, 1))
    ...
    def forward(self, state, action):
        ...
        output = self.cd2(state)
        output = output.reshape((state.shape[0], state.shape[3]))
        output = self.net(torch.cat((output, action), dim=1))
        return output  # Q value
