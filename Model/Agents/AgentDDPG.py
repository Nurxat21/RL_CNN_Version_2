...
from Model.Agents.AgentBase import AgentBase
from Model.Evaluate.net import Actor, Critic
...
class AgentDDPG(AgentBase):
    ...
    def init(self, net_dim=256, state_dim=8, action_dim=2, cd2_dim=32, reward_scale=1.0, gamma=0.99,
             learning_rate=1e-4, if_per_or_gae=False, env_num=1, gpu_id=0):
        """
        Explict call ``self.init()`` to overwrite the ``self.object`` in ``__init__()`` for multiprocessing.
        """
        AgentBase.init(self, net_dim=net_dim, state_dim=state_dim, action_dim=action_dim, cd2_dim=cd2_dim,
